...
subtype = "basal" #"classical" "normal" "primitive" "secretory"
...
library(data.table)
########################DATA
data=fread(paste("/home/mdiaz/workspace/LUSC",
...
penalty=c(CpGs=0.01,transcripts=0.01,miRNAs=0.07)#output of choose_penalty.R
...
penalty=c(CpGs=0.01,transcripts=0.01,miRNAs=0.07)
...
# to run normal subtype use this code to avoid problems with next functions
# Definir la función
# obtener_filas_no_cero <- function(df) {
#   # Obtener filas que tienen valores diferentes de cero en ambas columnas
#   filas_no_cero <- df[df$comp1 != 0 | df$comp2 != 0, ]
#   # Devolver el resultado
#   return(filas_no_cero)
# }
# 
# df_tr <- obtener_filas_no_cero(as.data.frame(final$loadings$transcripts))
# df_mr <- obtener_filas_no_cero(as.data.frame(final$loadings$miRNAs))
# 
# ########### transcripts
# transcripts <- list()
# names <- colnames(df_tr)
# temp <- list()
# for (i in seq_along(names)) {
#   temp[[i]] <-  df_tr[i]
#   names(temp)[i] <- names[i]
# }
# temp
# 
# transcripts[1] <- as.vector(temp$comp1)
# transcripts[2] <- as.vector(temp$comp2)
# names(transcripts) <- c("comp1", "comp2")
# 
# names(transcripts[1]$comp1) <- rownames(temp$comp1)
# names(transcripts[2]$comp2) <- rownames(temp$comp2)
# transcripts
# 
...
# temp
# 
# miRNAs[1] <- as.vector(temp$comp1)
# miRNAs[2] <- as.vector(temp$comp2)
# names(miRNAs) <- c("comp1", "comp2")
# 
# names(miRNAs[1]$comp1) <- rownames(temp$comp1)
# names(miRNAs[2]$comp2) <- rownames(temp$comp2)
# selected$miRNAs <- miRNAs
