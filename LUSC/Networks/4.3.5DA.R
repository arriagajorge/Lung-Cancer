...
#################RNA###########################################
subtype=read.table("../LUSC/subtypeLUSC.tsv",header=T,sep='\t')
expre=fread("../LUSC/RNAseqnormalized.tsv")
...
contr.mtrx=makeContrasts(
  ...
  primitive=primitive-normal,
  secretory=secretory-normal,
  levels=design)
...
#next:GSEA
#################miRNA###########################################
mir=fread("../LUSC/miRNAseqNormi.tsv")
...
#################METHYLATION###########################################
#get covariates
clin <- TCGAbiolinks::GDCquery_clinic("TCGA-LUSC","clinical")
colnames(subtype)[3]="tissue"
...
subtype=subtype[,c("submitter_id",
                   ...
                   "subtype",
                   "age_at_diagnosis.x",
                   "treatments_pharmaceutical_treatment_or_therapy.x",
                   "treatments_radiation_treatment_or_therapy.x",
                   "race.x",
                   "ajcc_pathologic_m.x",
                   "ajcc_pathologic_n.x",
                   "morphology.x",
...
write.table(subtype,"../LUSC/subtype.tsv",sep='\t',quote=F,row.names=F)
methy=data.table::fread("/home/mdiaz/workspace/LUSC/multiomics/methyM.tsv")
...
design=model.matrix(~0+subtype+age_at_diagnosis.x+morphology.x+
                      ajcc_pathologic_m.x+ajcc_pathologic_n.x+ajcc_pathologic_t.x+
                      primary_diagnosis.x+ajcc_pathologic_stage.x,subtype)
...
# [,1]   [,2]   [,3]   [,4]
# FALSE  89851 148555 283108  99714
# TRUE  328607 269903 135350 318744
...
fRUVfit <- function(x){
  return(RUVfit(Y=methy[,subtype$subtype%in%unique(subtype$subtype)[c(x,5)]],
                X=factor(subtype$subtype[subtype$subtype%in%unique(subtype$subtype)[c(x,5)]]),
                ctl=ctlimma[[x]]))
}
rf1 <- fRUVfit(1); rf2 <- fRUVfit(2); rf3 <- fRUVfit(3); rf4 <- fRUVfit(4) #fails with lapply

#adjust for unwanted variation #same fails with lapply
r1 <- RUVadj(Y=methy[,subtype$subtype%in%unique(subtype$subtype)[c(1,5)]], fit=rf1)
r2 <- RUVadj(Y=methy[,subtype$subtype%in%unique(subtype$subtype)[c(2,5)]], rf2)
r3 <- RUVadj(Y=methy[,subtype$subtype%in%unique(subtype$subtype)[c(3,5)]], fit=rf3)
r4 <- RUVadj(Y=methy[,subtype$subtype%in%unique(subtype$subtype)[c(4,5)]], fit=rf4)
rfit <- list(r1, r2, r3,r4)
