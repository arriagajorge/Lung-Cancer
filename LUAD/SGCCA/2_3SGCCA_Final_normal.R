...
final=wrapper.sgcca(X=transposeData(data),penalty=penalty,scale=F,
                    scheme="centroid",ncomp=4)#ncomp to explain 50% of transcripts matrix according to mfa.R
#get selected features
selected=lapply(final$loadings,function(y)
  apply(y,2,function(x) x[x!=0]))

miRNAs <-list()
miRNAs$comp1 <- c('hsa-mir-21' = -1)
miRNAs$comp2 <- c('hsa-mir-143' = 1)
miRNAs$comp3 <- c('hsa-mir-22' = -1)
miRNAs$comp4 <- c('hsa-mir-148a' = 1)
